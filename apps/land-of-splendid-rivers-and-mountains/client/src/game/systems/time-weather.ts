...
const WEATHER_PROBABILITIES: Readonly<Record<Season, ReadonlyArray<{ weather: Weather; weight: number }>>> = {
  spring: [
...
const WEATHER_OVERLAY_TINT: Readonly<Record<Weather, { color: number; extraAlpha: number }>> = {
...
export class TimeWeatherSystem implements IGameSystem {
  ...
  update(_scene: Phaser.Scene, _time: number, delta: number): void {
    ...
    const minutesElapsed = deltaSeconds * GAME_CONSTANTS.MINUTES_PER_REAL_SECOND
  ...
  setState(day: number, season: Season, year: number, timeOfDay: number, weather?: Weather): void {
    this.day = day
    this.season = season
    this.year = year
  ...
  private updateBrightness(): void {
    ...
    if (currentMinute >= last.minute) {
      alpha = last.alpha; r = last.r; g = last.g; b = last.b
    } else if (currentMinute < first.minute) {
      alpha = first.alpha; r = first.r; g = first.g; b = first.b
    } else {
    ...
    const finalR = Math.min(255, r + ((tint.color >> 16) & 0xff))
    const finalG = Math.min(255, g + ((tint.color >> 8) & 0xff))
    const finalB = Math.min(255, b + (tint.color & 0xff))
