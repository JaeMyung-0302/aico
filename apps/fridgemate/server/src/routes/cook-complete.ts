...
cookCompleteRouter.post('/:fridgeId/cook-complete', async (req, res: Response): Promise<void> => {
  ...
  if (
    ...
    !usedIngredients.every((n): n is string => typeof n === 'string' && n.trim().length > 0 && n.length <= 100)
  ) {
  ...
  try {
    ...
    console.error('Cook complete error:', error instanceof Error ? error.message : error)
