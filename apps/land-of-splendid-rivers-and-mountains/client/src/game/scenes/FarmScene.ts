...
import { FermentationSystem } from '../systems/fermentation'
...
export class FarmScene extends Phaser.Scene {
  ...
  create(): void {
    ...
    const cleanupEvents = registerFarmEventHandlers({
      inventory,
      energySystem,
      npcSystem,
      fermentSystem,
      ...
      eventSystem,
      syncInventory,
      renderBuildings: (bs) => this.renderBuildings(bs),
      buildingLayout: BUILDING_LAYOUT,
    })

    const syncFermentation = () => syncFermentationState(fermentSystem)
    const syncBuildings = () => syncBuildingsState(buildingSystem, (bs) => this.renderBuildings(bs))
    const syncAnimals = () => syncAnimalsState(animalSystem)
    const syncCombatHp = () => syncCombatHpState(combatSystem)
    ...
    saveManager.setup(
      () => {
        ...
        return {
          ...
          activeQuests: this.questSystem.getState().activeQuests,
          completedQuests: this.questSystem.getState().completedQuests,
          questProgress: this.questSystem.getState().progress,
          prologueCompleted: this.questSystem.isPrologueCompleted(),
        }
      },
      (data) => {
        ...
        this.questSystem?.loadState({
          ...
          progress: { ...(data.questProgress ?? {}) } as Record<string, number[]>,
          prologueCompleted: data.prologueCompleted ?? false,
        })
        const store = useGameStore.getState()
    ...
    this.questSystem = new QuestSystem(eventBus, storyData as unknown as QuestDefinition[])
    ...
    if (saveManager.hasSave()) {
      ...
      syncInventory()
      syncCombatHp()
      this.questSystem.startPrologue()
      eventBus.emit('prologue:dialogue', { questId: 'prologue-arrival', trigger: 'start' })
    }
