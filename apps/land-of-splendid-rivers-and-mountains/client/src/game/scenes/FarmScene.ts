...
const BUILDING_LAYOUT: Readonly<
  Record<string, { tileX: number; tileY: number; pixelSize: number }>
> = {
...
export class FarmScene extends Phaser.Scene {
  ...
  private readonly buildingSprites: Map<string, Phaser.GameObjects.Image> =
    new Map();
  ...
  private updateTileCursor(): void {
    ...
    if (
      tileX !== this.cursorTileX ||
      tileY !== this.cursorTileY ||
      color !== this.cursorColor ||
      alpha !== this.cursorAlpha
    ) {
      this.tileCursor.clear();
