...
const FLOOR_MONSTERS: ReadonlyArray<ReadonlyArray<{ id: string; x: number; y: number }>> = [
  [
    { id: 'cheuksin', x: 5, y: 5 },
...
export class DungeonScene extends Phaser.Scene {
  ...
  preload(): void {
    ...
    this.load.tilemapTiledJSON('mountain-map', '/assets/tilemaps/mountain.json')
  }
  ...
  create(): void {
    ...
    const syncCombatHp = () => {
      useGameStore.getState().setPlayerHp(this.combatSystem.getHp(), this.combatSystem.getMaxHp())
    }
    ...
    const onFainted = ({ goldPenaltyRate }: { goldPenaltyRate: number }) => {
      ...
      if (penalty > 0) {
        eventBus.emit('gold:changed', { amount: -penalty, total: s.gold - penalty })
      }
    }
  ...
  update(_time: number, delta: number): void {
    ...
    if (inputState.action && this.combatSystem.isAlive()) {
      ...
      if (this.energySystem.consume('COMBAT') || cost === 0) {
        ...
        const d = dirMap[dir]
        this.combatSystem.performAttack(this.player.sprite.x, this.player.sprite.y, d.x, d.y)
      }
    }
  ...
  private spawnFloorMonsters(): void {
    ...
    for (const spawn of floorConfig) {
      const x = spawn.x * GAME_CONSTANTS.TILE_SIZE + GAME_CONSTANTS.TILE_SIZE / 2
      const y = spawn.y * GAME_CONSTANTS.TILE_SIZE + GAME_CONSTANTS.TILE_SIZE / 2
      if (this.combatSystem.spawnMonster(spawn.id, x, y)) {
        this.monstersAlive++
      }
    }
    this.floorCleared = false
  ...
  private completeDungeon(): void {
    ...
    const store = useGameStore.getState()
    const reward = 500
    eventBus.emit('gold:changed', { amount: reward, total: store.gold + reward })
  ...
  private createTilesetTexture(): void {
    ...
    const canvas = this.textures.createCanvas('tileset-mountain', size * 2, size * 2)
    if (!canvas) return
