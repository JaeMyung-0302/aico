    ...
    return (
      <div style={styles.overlay}>
        <div style={styles.rewardBox}>
          ...
          <div style={styles.rewardName}>{t(`${reward.eventId}.name` as never) || reward.eventId}</div>
          {reward.goldReward > 0 && (
            <div style={styles.rewardGold}>+{reward.goldReward}G</div>
          )}
          {reward.items.map((item, i) => (
            <div key={i} style={styles.rewardItem}>
              {t(`item.food.${item.itemId.replace('food-', '')}` as never) || item.itemId} x{item.quantity}
            </div>
          ))}
          <button type="button" style={styles.rewardBtn} onClick={dismissReward}>
            {t('ui.event.close')}
          </button>
        </div>
      </div>
  ...
  return (
    <div style={styles.banner}>
      ...
      <div style={styles.bannerText}>
        <div style={styles.bannerTitle}>{t(`event.${activeEventId}` as never)}</div>
        <div style={styles.bannerDesc}>{t(`event.${activeEventId}.desc` as never)}</div>
      </div>
    </div>
  )
}
...
const styles: Record<string, React.CSSProperties> = {
  banner: {
    ...
    alignItems: 'center',
    gap: 8,
    background: 'linear-gradient(135deg, rgba(139, 69, 19, 0.9), rgba(184, 115, 51, 0.9))',
    borderRadius: 8,
    padding: '6px 14px',
    color: '#fff',
