...
import { SUBSCRIPTION_PRICE, SUBSCRIPTION_DAYS, MAX_RETRY_COUNT } from './constants';
...
export class RenewalService {
  ...
  private processActiveRenewals = async () => {
    ...
    const subscriptions = await this.prisma.subscription.findMany({
      ...
      select: { id: true, userId: true, billingKey: true, currentPeriodEnd: true, retryCount: true },
    });

    this.logger.log(`갱신 대상 ACTIVE 구독: ${subscriptions.length}건`);

    for (const sub of subscriptions) {
      await this.renewSubscription(sub.id, sub.userId, sub.billingKey, sub.currentPeriodEnd, sub.retryCount);
    }
  };
  ...
  private renewSubscription = async (
    ...
    try {
      ...
      const newPeriodEnd = new Date(currentPeriodEnd.getTime() + SUBSCRIPTION_DAYS * 24 * 60 * 60 * 1000);

      await this.prisma.$transaction([
        this.prisma.subscription.update({
          where: { id: subscriptionId },
          data: { status: 'ACTIVE', currentPeriodEnd: newPeriodEnd, retryCount: 0 },
        }),
      ...
      this.logger.error(`갱신 실패: subscription=${subscriptionId}, code=${errorCode}`);
      ...
      try {
        ...
        if (isExpired) {
          this.logger.warn(`구독 만료: subscription=${subscriptionId} (${MAX_RETRY_COUNT}회 실패)`);
        }
      } catch (dbError) {
        const dbErrorCode = dbError instanceof Error ? dbError.message.slice(0, 100) : 'UNKNOWN';
        this.logger.error(`갱신 실패 기록 중 에러: subscription=${subscriptionId}, code=${dbErrorCode}`);
      }
    }
  };
}
