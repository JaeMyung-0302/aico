...
describe('CharactersService', () => {
  ...
  describe('findByClassType', () => {
    it('should throw BadRequestException for invalid class type', async () => {
      await expect(service.findByClassType('user-1', 'Invalid')).rejects.toThrow(BadRequestException);
    });

    it('should throw NotFoundException when character not found', async () => {
      mockPrisma.character.findFirst.mockResolvedValue(null);

      await expect(service.findByClassType('user-1', 'Warrior')).rejects.toThrow(NotFoundException);
    });
    ...
    it('should accept all valid class types', async () => {
      ...
      for (const type of validTypes) {
        mockPrisma.character.findFirst.mockResolvedValue({ id: 'c1', classType: type });
  ...
  describe('upsert', () => {
    it('should throw BadRequestException for invalid class type', async () => {
      await expect(service.upsert('user-1', 'Invalid', {})).rejects.toThrow(BadRequestException);
    });

    it('should upsert character with partial stats', async () => {
      mockPrisma.character.upsert.mockResolvedValue({ id: 'c1', permanentHp: 5 });

      const result = await service.upsert('user-1', 'Warrior', { permanentHp: 5 });
