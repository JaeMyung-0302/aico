...
export class UpgradesService {
  ...
  purchaseUpgrade = async (userId: string, dto: PurchaseUpgradeDto) => {
    ...
    const camelKey = `permanent${dto.statType.charAt(0).toUpperCase()}${dto.statType.slice(1)}` as keyof typeof characters[0];
    ...
    await this.prisma.$transaction(async (tx) => {
      try {
        ...
        throw new BadRequestException({ error: 'Insufficient meta gold', required: cost });
      }
