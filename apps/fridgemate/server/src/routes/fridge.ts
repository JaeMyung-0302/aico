...
const VALID_COMPARTMENT_TYPES: Set<string> = new Set(
  Object.values(CompartmentType),
)
const MAX_COMPARTMENTS = 50

const validateCompartments = (
  items: unknown[],
): items is CompartmentPreset[] => {
  if (items.length > MAX_COMPARTMENTS) return false
  return items.every((c: unknown) => {
    if (typeof c !== 'object' || c === null) return false
    const obj = c as Record<string, unknown>
    return (
      typeof obj['type'] === 'string' &&
      VALID_COMPARTMENT_TYPES.has(obj['type']) &&
...
fridgeRouter.post(
  ...
  async (req, res: Response): Promise<void> => {
    ...
    const compartmentData =
      compartments && compartments.length > 0 ? compartments : presets
...
fridgeRouter.put(
  ...
  async (req, res: Response): Promise<void> => {
    ...
    try {
      // 그룹 소유권 확인
      const targetFridge = await prisma.fridge.findUnique({
        where: { id: fridgeId },
      })
      ...
      const occupiedIds = existing
        .filter((c) => c.foodItems.length > 0)
        .map((c) => c.id)
      if (occupiedIds.length > 0) {
        res.status(400).json({
          error:
            '식재료가 있는 칸이 있습니다. 식재료를 이동한 후 다시 시도해주세요.',
