...
const AI_KEYWORDS = [
  "ai", "인공지능", "chatgpt", "gpt", "claude", "gemini",
  "llm", "딥러닝", "머신러닝", "생성ai", "copilot",
  "midjourney", "sora", "openai", "anthropic", "google ai",
  "ai 모델", "ai 도구", "ai 서비스", "ai 업데이트",
  "뤼튼", "클로드", "제미나이",
];
...
export const fetchGoogleTrends = async (): Promise<Keyword[]> => {
  try {
    ...
    console.log(`[Google Trends] AI 필터: ${allItems.length}개 → ${filtered.length}개 통과`);
    return filtered;
  } catch (error) {
