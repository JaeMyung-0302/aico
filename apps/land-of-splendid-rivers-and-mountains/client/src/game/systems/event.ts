...
import type { Season, EventDefinition } from '@land-of-splendid-rivers-and-mountains/shared'
import { eventBus } from '@/lib/event-bus'
import type { IGameSystem } from './system-manager'
import eventsData from '../data/events.json'

const eventDefs: ReadonlyArray<EventDefinition> = eventsData as ReadonlyArray<EventDefinition>
...
export class EventSystem implements IGameSystem {
  ...
  private onDayEnd = ({ day, season }: { day: number; season: Season; year: number }) => {
    this.currentDay = day
  ...
  setState(completedEvents: ReadonlyArray<string>, activeEventId: string | null): void {
  ...
  private checkEvents(): void {
    if (this.activeEventId) {
      ...
      if (activeDef) {
        ...
        if (this.currentDay >= endDay || this.currentSeason !== activeDef.season) {
