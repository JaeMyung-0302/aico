...
export class KamisService implements OnModuleInit {
  ...
  private refreshCache = async (): Promise<void> => {
    ...
    for (const categoryCode of CATEGORIES) {
      try {
        ...
        if (items.length > 0) {
          ...
          this.logger.log(`KAMIS 카테고리 ${categoryCode}: ${items.length}개 품목 캐시`);
        }
        // rate limit 방지
    ...
    const totalItems = [...itemCache.values()].reduce((sum, arr) => sum + arr.length, 0);
    this.logger.log(`KAMIS 캐시 완료: 총 ${totalItems}개 품목`);
  };

  // 카테고리별 일일 가격 조회
  private fetchCategory = async (categoryCode: string, regday: string): Promise<CachedItem[]> => {
    ...
    for (const item of rawItems) {
      ...
      const price = this.parsePrice(item.dpr1) ?? this.parsePrice(item.dpr2) ?? this.parsePrice(item.dpr3);
  ...
  private normalizeIngredientName = (name: string): string => {
    return name
      .replace(/^(다진|썬|채썬|슬라이스|간|갈은|삶은|데친|볶은|구운|튀긴)\s?/, '')
      .trim();
  };
  ...
  findMatch = (ingredientName: string): CachedItem | null => {
    ...
    // 1. 정확히 일치
    const exact = allItems.find((item) => item.itemName === name || item.itemName === cleanName);
    ...
    for (const item of sortedByNameLength) {
      if (cleanName.includes(item.itemName) || cleanName.includes(this.normalizeItemName(item.itemName))) {
        return item;
      }
    }
    ...
    if (altNames) {
      for (const alt of altNames) {
        const found = allItems.find(
          (item) => item.itemName === alt || this.normalizeItemName(item.itemName) === alt,
        );
