...
const signToken = (userId: string, groupId: string | null, email: string): string =>
  jwt.sign({ userId, groupId, email }, process.env.JWT_SECRET!, { expiresIn: JWT_EXPIRES_IN })
...
authRouter.post('/register', authRateLimiter, async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    res.json({ token, user: { id: user.id, email: user.email, name: user.name, groupId: null } })
  } catch {
...
authRouter.post('/login', authRateLimiter, async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    res.json({ token, user: { id: user.id, email: user.email, name: user.name, groupId: user.groupId } })
  } catch {
...
authRouter.post('/group/join', joinRateLimiter, async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    payload = jwt.verify(authHeader, process.env.JWT_SECRET!) as { userId: string; email: string }
  } catch {
    res.status(401).json({ error: 'Invalid or expired token' })
    return
  }
  ...
  try {
    ...
    const existingRequest = await prisma.joinRequest.findUnique({
      where: { userId_groupId: { userId: payload.userId, groupId: group.id } },
    })
...
authRouter.post('/group/create', async (req: Request, res: Response): Promise<void> => {
  const authHeader = req.headers.authorization?.replace('Bearer ', '')
  if (!authHeader) {
    res.status(401).json({ error: 'Authorization token required' })
    return
  ...
  try {
    ...
    for (let i = 0; i < MAX_RETRIES; i++) {
      ...
      if (!existing) break
      if (i === MAX_RETRIES - 1) {
        res.status(500).json({ error: '그룹 코드 생성에 실패했습니다. 잠시 후 다시 시도해주세요' })
        return
      }
    }
    ...
    res.json({
      token,
      user: { id: user.id, email: user.email, name: user.name, groupId: group.id },
      group: { id: group.id, name: group.name, code: group.code },
    })
  } catch (err) {
    if (err && typeof err === 'object' && 'code' in err && err.code === 'P2002') {
      res.status(409).json({ error: '그룹 코드 충돌이 발생했습니다. 다시 시도해주세요' })
      return
    }
