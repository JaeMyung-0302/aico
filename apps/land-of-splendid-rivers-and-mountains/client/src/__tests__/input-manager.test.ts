...
import { InputManager, type InputState } from '../game/input/input-manager'
...
describe('InputManager', () => {
  ...
  describe('state updates', () => {
    it('should update movement from provider state', () => {
      inputManager.updateFromProvider({ moveX: 1, moveY: 0, action: false, cancel: false, interact: false })
      const state = inputManager.getState()
    ...
    it('should normalize diagonal movement', () => {
      inputManager.updateFromProvider({ moveX: 1, moveY: 1, action: false, cancel: false, interact: false })
      const state = inputManager.getState()
    ...
    it('should not normalize when movement is zero', () => {
      inputManager.updateFromProvider({ moveX: 0, moveY: 0, action: false, cancel: false, interact: false })
      const state = inputManager.getState()
      expect(state.moveX).toBe(0)
      expect(state.moveY).toBe(0)
    })
    ...
    it('should update action states', () => {
      inputManager.updateFromProvider({ moveX: 0, moveY: 0, action: true, cancel: false, interact: true })
      const state = inputManager.getState()
  ...
  describe('reset', () => {
    it('should reset all state to defaults', () => {
      inputManager.updateFromProvider({ moveX: 1, moveY: -1, action: true, cancel: true, interact: true })
      inputManager.reset()
      const state = inputManager.getState()
      expect(state.moveX).toBe(0)
      expect(state.moveY).toBe(0)
