import type { Season, Weather, FarmPlotState, FermentationState, BuildingState, AnimalState, InventorySlot, EquippedTools } from '@land-of-splendid-rivers-and-mountains/shared'
...
const REQUIRED_SAVE_FIELDS = [
  'version', 'gold', 'energy', 'maxEnergy', 'day', 'season',
  'year', 'timeOfDay', 'farmPlots', 'inventory', 'equippedTools',
] as const
...
export class SaveManager {
  ...
  patchSave(updates: Partial<SaveData>): void {
    try {
      ...
      localStorage.setItem(SAVE_KEY, JSON.stringify({ ...parsed, ...updates, lastSavedAt: new Date().toISOString() }))
    } catch { /* ignore */ }
  }
  ...
  private migrate(data: SaveData): SaveData {
    ...
    if (migrated.version === 6) {
      migrated = { ...migrated, playerHp: 50, playerMaxHp: 50, playerAttack: 5, playerDefense: 2, version: 7 }
    }
