...
export class Monster {
  ...
  constructor(scene: Phaser.Scene, x: number, y: number, def: MonsterDefinition, instanceId: string) {
    this._id = instanceId
    ...
    const key = scene.textures.exists(textureKey) ? textureKey : 'monster-default'
  ...
  takeDamage(damage: number, knockbackX: number, knockbackY: number, knockbackForce: number): boolean {
  ...
  reset(x: number, y: number, def: MonsterDefinition, instanceId: string): void {
    this._id = instanceId
    this._definition = def
    this.hp = def.hp
    this.spawnX = x
    ...
    const textureKey = `monster-${def.id}`
    const key = this.sprite.scene.textures.exists(textureKey) ? textureKey : 'monster-default'
  ...
  private chase(dx: number, dy: number, dist: number): void {
    ...
    const nx = dx / dist
    const ny = dy / dist
    this.sprite.setVelocity(nx * this.definition.speed, ny * this.definition.speed)
  }
  ...
  private createAnimations(scene: Phaser.Scene, textureKey: string): void {
    ...
    for (const [dir, start] of directions) {
      ...
      if (!scene.anims.exists(animKey)) {
        scene.anims.create({
          key: animKey,
          frames: scene.anims.generateFrameNumbers(textureKey, { start, end: start + 3 }),
  ...
  private patrol(delta: number): void {
    ...
    if (this.patrolTimer <= 0) {
      if (this.aiState === 'idle') {
        ...
        this.aiState = 'idle'
        this.sprite.setVelocity(0, 0)
        this.patrolPause = PATROL_PAUSE_MIN + Math.random() * (PATROL_PAUSE_MAX - PATROL_PAUSE_MIN)
