...
import type { MonsterDefinition, WeaponDefinition } from '@land-of-splendid-rivers-and-mountains/shared'
...
export const getWeaponDef = (id: string): WeaponDefinition | undefined => weaponDefs.get(id)
export const getMonsterDef = (id: string): MonsterDefinition | undefined => monsterDefs.get(id)
...
export class CombatSystem implements IGameSystem {
  ...
  update(_scene: Phaser.Scene, _time: number, delta: number): void {
    ...
    for (const [, monster] of this.monsters) {
      ...
      if (monster.canAttack() && this.invincibleTimer <= 0) {
        this.playerTakeDamage(monster.definition.attack, monster.sprite.x, monster.sprite.y)
  ...
  performAttack(playerX: number, playerY: number, dirX: number, dirY: number): boolean {
    ...
    for (const [id, monster] of this.monsters) {
      ...
      const defeated = monster.takeDamage(totalDamage, dx, dy, weapon.knockback)
  ...
  private playerTakeDamage(monsterAttack: number, fromX: number, fromY: number): void {
    if (!this.playerSprite) return
