...
export const getDb = (): Database.Database => {
  ...
  try {
    ...
    db.exec(
      `INSERT INTO keywords (keyword, source, score, used) VALUES ('__migration_test__', 'news', 0, 0)`,
    )
...
export const updatePostStatus = (
  ...
  if (url) {
    db.prepare('UPDATE posts SET status = ?, url = ? WHERE id = ?').run(
      status,
      url,
      postId,
    )
