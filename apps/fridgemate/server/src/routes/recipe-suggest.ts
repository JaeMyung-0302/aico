...
recipeSuggestRouter.post(
  '/:fridgeId/recipe-suggest',
  async (req, res: Response): Promise<void> => {
    ...
    try {
      ...
      if (allItems.length === 0) {
        res.status(400).json({
          error: '냉장고에 재료가 없습니다. 재료를 먼저 등록해주세요.',
        })
        return
      }
      ...
      if (selectedItemIds !== undefined) {
        if (
          ...
          !selectedItemIds.every(
            (id) => typeof id === 'string' && UUID_REGEX.test(id),
          )
        ) {
          res.status(400).json({
            error: 'selectedItemIds must be an array of valid UUIDs (max 100)',
          })
          return
        }
      ...
      if (targetItems.length === 0) {
        res
          .status(400)
          .json({ error: '선택한 재료가 냉장고에 존재하지 않습니다' })
        return
      }
      ...
      const foodItemInputs = targetItems.map((item) => {
        ...
        const daysUntilExpiry = item.expiryDate
          ? Math.floor(
              (new Date(item.expiryDate).getTime() - todayStart.getTime()) /
                (1000 * 60 * 60 * 24),
            )
          : null
