...
const mockHistorical = yahooFinance.historical as jest.MockedFunction<typeof yahooFinance.historical>;
...
describe('YahooProvider', () => {
  ...
  describe('fetchHistorical', () => {
    it('yahoo-finance2 historical을 올바른 파라미터로 호출해야 한다', async () => {
      mockHistorical.mockResolvedValueOnce([
        { date: new Date('2024-01-02'), open: 150, high: 155, low: 149, close: 153, volume: 1000000, adjClose: 153 },
      ] as any);
    ...
    it('실패 시 3회까지 재시도해야 한다', async () => {
      ...
      expect(mockHistorical).toHaveBeenCalledTimes(3);
      expect(result).toHaveLength(1);
    });
    ...
    it('PriceDataRow 형태로 변환해야 한다', async () => {
      mockHistorical.mockResolvedValueOnce([
        { date: new Date('2024-01-02'), open: 150.5, high: 155.3, low: 149.2, close: 153.8, volume: 1000000, adjClose: 153.8 },
      ] as any);
