...
export class NPCSystem implements IGameSystem {
  ...
  private spawnNpcsForLocation(scene: Phaser.Scene, location: string): void {
    ...
    for (const def of npcDefs) {
      const scheduleEntry = def.schedule?.find(
        (s) => s.season === this.currentSeason,
      );
      const effectiveLocation = scheduleEntry
      ...
      const npc = new NPC(scene, spawnX, spawnY, def, index);
  ...
  private readonly onSeasonChange = ({ season }: { season: Season }): void => {
    ...
    this.npcs.clear();
    this.spawnNpcsForLocation(this.currentScene, this.currentLocation);
  };
