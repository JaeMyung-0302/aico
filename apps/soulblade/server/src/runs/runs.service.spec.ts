...
const mockTx = {
  equipment: {
    createManyAndReturn: jest.fn().mockResolvedValue([{ id: 'eq-1' }, { id: 'eq-2' }]),
  },
  gameRun: {
...
describe('RunsService', () => {
  ...
  it('should throw ForbiddenException when character not owned', async () => {
    ...
    await expect(service.completeRun('user-1', validDto)).rejects.toThrow(ForbiddenException);
  });
  ...
  it('should throw BadRequestException for invalid stage', async () => {
    ...
    await expect(
      service.completeRun('user-1', { ...validDto, stageId: 'invalid_stage' as never }),
