...
import { Group, Mesh, BoxGeometry, CylinderGeometry, ConeGeometry } from 'three'
...
export const createBossModel = (): Group => {
  ...
  const mouth = new Mesh(
    new BoxGeometry(8, 3, HEAD_R + 1),
    buildMat(BOSS_COLORS.mouth),
  )
  ...
  const teeth = new Mesh(
    new BoxGeometry(6, 1.5, 1),
    buildMat(BOSS_COLORS.teeth),
  )
  ...
  const eyeOuter = (x: number) => {
    const m = new Mesh(
      ...
      buildMat(BOSS_COLORS.eyesOuter),
    )
  ...
  const eyeInner = (x: number) => {
    const m = new Mesh(
      new BoxGeometry(2, 1.5, 1.2),
      buildMat(BOSS_COLORS.eyesInner),
    )
  ...
  // Horns (main)
  const hornL = new Mesh(
    new ConeGeometry(HORN_R, HORN_H, HORN_SEG),
    buildMat(BOSS_COLORS.horn),
  )
  hornL.position.set(-6, HORN_Y, 0)
  group.add(hornL)
  const hornR = new Mesh(
    new ConeGeometry(HORN_R, HORN_H, HORN_SEG),
    buildMat(BOSS_COLORS.horn),
  )
  ...
  // Horns (small, outer)
  const smHornL = new Mesh(
    new ConeGeometry(HORN_SM_R, HORN_SM_H, HORN_SEG),
    buildMat(BOSS_COLORS.hornSmall),
  )
  smHornL.position.set(-10, HORN_Y - 2, 0)
  group.add(smHornL)
  const smHornR = new Mesh(
    new ConeGeometry(HORN_SM_R, HORN_SM_H, HORN_SEG),
    buildMat(BOSS_COLORS.hornSmall),
  )
  ...
  const belly = new Mesh(
    new BoxGeometry(BODY_W - 8, BODY_H - 4, BODY_D + 1),
    buildMat(BOSS_COLORS.belly),
  )
  ...
  // Shoulders
  const shL = new Mesh(
    new BoxGeometry(SHOULDER_W, SHOULDER_H, SHOULDER_D),
    buildMat(BOSS_COLORS.shoulder),
  )
