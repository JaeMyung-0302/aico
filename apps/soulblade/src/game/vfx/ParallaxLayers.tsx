...
export const ParallaxLayers = ({
  ...
  const farTexture = useMemo(() => {
    ...
    tex.repeat.set(
      (worldWidth + 3000) / TILE_SIZE,
      (worldHeight + 3000) / TILE_SIZE,
    )
    return tex
  ...
  const nearTexture = useMemo(() => {
    ...
    tex.repeat.set(
      (worldWidth + 2500) / TILE_SIZE,
      (worldHeight + 2500) / TILE_SIZE,
    )
    return tex
  }, [mapId, worldWidth, worldHeight, theme])
  ...
  useFrame(() => {
    ...
    if (farRef.current) {
      farRef.current.position.x =
        camX * (1 - FAR_FACTOR) + (worldWidth / 2) * FAR_FACTOR
      farRef.current.position.z =
        camZ * (1 - FAR_FACTOR) + (worldHeight / 2) * FAR_FACTOR
    }

    if (nearRef.current && layerCount >= 2) {
      nearRef.current.position.x =
        camX * (1 - NEAR_FACTOR) + (worldWidth / 2) * NEAR_FACTOR
      nearRef.current.position.z =
        camZ * (1 - NEAR_FACTOR) + (worldHeight / 2) * NEAR_FACTOR
    }
  })
  ...
  return (
    <group>
      ...
      <mesh
        ref={farRef}
        position={[worldWidth / 2, FAR_Y, worldHeight / 2]}
      ...
      {layerCount >= 2 && (
        <mesh
          ref={nearRef}
          position={[worldWidth / 2, NEAR_Y, worldHeight / 2]}
          rotation={[-Math.PI / 2, 0, 0]}
        >
